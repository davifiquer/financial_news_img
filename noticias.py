...
def calendario(url):
    ...
    requisicao = requests.get(url, headers=cabecalho)  # requisiçao dentro do site

    # processo de requisiçao de dados no site <--

    # processo de tratamento do html -->

    soup = BeautifulSoup(requisicao.text, 'html.parser')  # tratamento do html com o módulo 'bs4'
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if impacto == 2 or impacto == 3:
            # objeto_datetime = datetime.strptime(horario, '%H:%M')
            # objeto_datetime.strftime('%H:%M')
            # hora_agora = datetime.now()
            # if objeto_datetime.time() > hora_agora.time():
            #     print('A hora é maior!')
            # else:
            #     print('A hora é menor!')
